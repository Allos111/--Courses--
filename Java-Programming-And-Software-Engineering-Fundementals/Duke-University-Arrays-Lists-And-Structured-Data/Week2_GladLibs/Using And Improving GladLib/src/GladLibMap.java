...
public class GladLibMap
{
    ...
    private void initializeFromSource(String source)
    {
        ...
        {
            words = readIt(source + "/" + category[i] + ".txt");
            myMap.put(category[i], words);
        }
     ...
     * It's also checks the next value inside the source
     */
...
//    }
    private int totalWordsInMap()
    {
        ...
        for (String word: myMap.keySet())
        {
            ArrayList<String> words = myMap.get(word);
            sum += words.size();
        }
        return sum;
    }
    ...
    {
        GladLibMap tellStory  = new GladLibMap();
        tellStory.makeStory();
    }
}
